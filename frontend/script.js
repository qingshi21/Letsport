...
function openLoginModal() {
    console.log('openLoginModal被调用，当前登录状态:', API.utils.isLoggedIn());
    console.log('当前用户token:', API.utils.getToken());
...
// 全局登录状态检查函数
function checkLoginAndProceed(callback, showLoginModal = true) {
    if (!API.utils.isLoggedIn()) {
        if (showLoginModal) {
            alert('请先登录');
            openLoginModal();
...
navLinks.forEach(link => {
    link.addEventListener('click', function(e) {
        ...
        console.log('当前登录状态:', API.utils.isLoggedIn());
        ...
        if (this.classList.contains('login-btn')) {
            ...
            console.log('登录按钮被点击，当前登录状态:', API.utils.isLoggedIn());
